RSpec.describe Peatio::FantasyGold::Client do
  let(:uri) { "http://user:password@127.0.0.1:57810" }
  let(:uri_without_authority) { "http://127.0.0.1:57810" }

  before(:all) { WebMock.disable_net_connect! }
  after(:all) { WebMock.allow_net_connect! }

  subject { Peatio::FantasyGold::Client.new(uri) }
    ...
    context :methodnotfound do
      ...
      it do
        expect{ subject.json_rpc(:methodnotfound) }.to \
          raise_error(Peatio::FantasyGold::Client::ResponseError, "Method not found (-32601)")
      end
    end
    ...
    context :notfound do
      ...
      it do
        expect{ subject.json_rpc(:notfound) }.to \
          raise_error(Peatio::FantasyGold::Client::Error)
      end
    end
    ...
    context :connectionerror do
      ...
      it do
        expect{ subject.json_rpc(:connectionerror) }.to \
          raise_error(Peatio::FantasyGold::Client::ConnectionError)
      end
    end
  end
end
